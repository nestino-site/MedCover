...
async function ContentPageBody({ slugPath }: { slugPath: string }) {
  ...
  return (
    <>
      ...
      <div className="mx-auto max-w-4xl px-4 pb-16 sm:px-6 lg:px-8">
        ...
        {page.htmlContent && (
          <ContentHtml html={page.htmlContent} className="mt-8" />
        )}

        {page.tableOfContents.length > 0 && (
          <RelatedPages toc={page.tableOfContents} />
        )}
...
export default async function CatchAllPage({ params }: { params: Params }) {
  ...
  return (
    <Suspense fallback={<PageSkeleton />}>
      <ContentPageBody slugPath={slugPath} />
    </Suspense>
  )
}
